...
def visualize_loss(plot, stats):
    """
    plot - a matplotlib.pyplot object
    stats - the output of the 'train' method of two layer net
    """
    plot.subplot(2, 1, 1)
    plot.plot(stats['loss_history'])
    plot.title('Loss history')
    plot.xlabel('Iteration')
    plot.ylabel('Loss')

    plot.subplot(2, 1, 2)
    plot.plot(stats['train_acc_history'], label='train')
    plot.plot(stats['val_acc_history'], 'r--', label='val')
    plot.title('Classification accuracy history')
    plot.xlabel('Epoch')
    plot.ylabel('Clasification accuracy')
    plot.show()
...
class TwoLayerNet(object):
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in xrange(num_iters):
            rand_indexes = np.random.choice(N, batch_size)
